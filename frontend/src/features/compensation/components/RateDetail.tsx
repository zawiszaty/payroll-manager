...
import { AlertCircle, ArrowLeft, History, DollarSign } from 'lucide-react'
...
export function RateDetail() {
  ...
  const [rate, setRate] = useState<RateView | null>(null)
  ...
  useEffect(() => {
    const loadAuditHistory = async () => {
      ...
      try {
        ...
        const logs = await auditApi.getByEntity('rate', id)
        setAuditLogs(logs)
      } catch (err) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-muted-foreground">Loading rate details...</div>
      </div>
    )
  }
    ...
    return (
      <div className="space-y-4">
        ...
        <Alert variant="destructive">
          ...
          <AlertDescription>{error || 'Rate not found'}</AlertDescription>
        </Alert>
      </div>
    )
  }
        ...
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-2xl font-bold">{formatCurrency(rate.amount, rate.currency)}</p>
            </div>

            <div className="space-y-2">
              <p className="text-sm font-medium text-muted-foreground">Valid From</p>
              <p className="text-base">{formatDate(rate.valid_from)}</p>
            </div>

            <div className="space-y-2">
              <p className="text-sm font-medium text-muted-foreground">Valid To</p>
              <p className="text-base">{formatDate(rate.valid_to) || 'Ongoing'}</p>
            </div>

            {rate.description && (
              <div className="space-y-2 col-span-2">
                <p className="text-sm font-medium text-muted-foreground">Description</p>
                <p className="text-base">{rate.description}</p>
              </div>
            )}

            <div className="space-y-2">
              <p className="text-sm font-medium text-muted-foreground">Created At</p>
              <p className="text-base">{formatDate(rate.created_at)}</p>
            </div>

            <div className="space-y-2">
              <p className="text-sm font-medium text-muted-foreground">Updated At</p>
              <p className="text-base">{formatDate(rate.updated_at)}</p>
            </div>
          </div>
        </CardContent>
      </Card>
